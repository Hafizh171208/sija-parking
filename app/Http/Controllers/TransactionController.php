...
class TransactionController extends Controller
{
    ...
    public function processExit(Request $request)
    {
        ...
        $total_menit = ceil($totalDurasiDetik / 60);
        if ($total_menit <= 0) {
            $total_menit = 1;
        }
        if ($total_menit == 1) {
            ...
            $total_bayar = $tarif->perjam_pertama + (($total_menit - 1) * $tarif->perjam_berikutnya);
        }
        if ($total_bayar > $tarif->max_perhari) {
            $total_bayar = $tarif->max_perhari;
        }
        $transaction->no_polisi = strtoupper(trim($request->no_polisi));
        $transaction->keluar = $waktuKeluar;
        $transaction->perjam_pertama = $tarif->perjam_pertama;
        $transaction->perjam_berikutnya = $tarif->perjam_berikutnya;
        $transaction->max_perhari = $tarif->max_perhari;
        $transaction->total_jam = $total_menit;
